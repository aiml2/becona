...
#adamw = AdamW(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0., weight_decay=0.025, batch_size=32, samples_per_epoch=1, epochs=1)


#Same as Xecptionv3 but with Adam for all epochs
#   frozen till layer 115 in era 1
#Use typical steps_per_epoch/validation_steps cfr. docs & count.py
class FTMC_Xception_v7(AbstractFineTuneModelConfig):
...
# Same fit_generator?
        ...
        savename='BECONA'+str(self.dataSetVersion)+self.configId+"_split"+str(cvIndex)+eraName+"_15+ep{epoch:02d}.hdf5"
        checkpointer = ModelCheckpoint(filepath=checkpointName+self.checkpointEnd, verbose=1, save_best_only=True)
        self.model.compile(optimizer=Adam(lr=0.00001), loss='categorical_crossentropy')
        self.model.fit_generator(
                trainGen,
                steps_per_epoch=self.steps_per_epoch,
                epochs=10,
        ...
        savename='BECONA'+str(self.dataSetVersion)+self.configId+"_split"+str(cvIndex)+eraName+"_25+ep{epoch:02d}.hdf5"
        checkpointer = ModelCheckpoint(filepath=checkpointName+self.checkpointEnd, verbose=1, save_best_only=True)
        self.model.compile(optimizer=Adam(lr=0.000001), loss='categorical_crossentropy')
        ...
        self.model.save(savename.format(epoch=40))
        pass
